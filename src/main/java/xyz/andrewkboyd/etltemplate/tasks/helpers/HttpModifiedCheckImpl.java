...
public class HttpModifiedCheckImpl implements HttpModifiedCheck {
     ...
     */
    @Override
    public CompletableFuture<Boolean> isNewDataAvailable(ZonedDateTime lastFetchTime, URI resourceUri) {
        CompletableFuture<Boolean> fut = new CompletableFuture<>();
        Disposable disposable = webClientBuilder.build()
                .head()
                .uri(resourceUri)
                .exchangeToFlux(clientResponse -> {
                    ...
                    ZonedDateTime lastModifiedParsed = ZonedDateTime.parse(lastModified, DateTimeFormatter.RFC_1123_DATE_TIME);
